  ...
  Widget _buildTopCities(BuildContext context) {
    return Consumer<HotelProvider>(
      builder: (context, provider, child) {
        // Extract unique city names — use the part before the comma if present
        final uniqueCities = provider.allHotels
            .map((h) {
              final loc = h.city.isNotEmpty ? h.city : h.location;
              // Take only the city part before any comma e.g. "Paris, France" → "Paris"
        ...
        if (uniqueCities.isEmpty) return const SizedBox.shrink();

        // Map city names to representative Unsplash images
        String _cityImage(String city) {
          final c = city.toLowerCase();
          if (c.contains('london'))    return 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?q=80&w=400';
          if (c.contains('paris'))     return 'https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=400';
          if (c.contains('new york') || c.contains('york')) return 'https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?q=80&w=400';
          if (c.contains('tokyo'))     return 'https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?q=80&w=400';
          if (c.contains('dubai'))     return 'https://images.unsplash.com/photo-1512453979798-5ea266f8880c?q=80&w=400';
          if (c.contains('bali'))      return 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=400';
          if (c.contains('maldives') || c.contains('malé')) return 'https://images.unsplash.com/photo-1573843981267-be1999ff37cd?q=80&w=400';
          if (c.contains('santorini') || c.contains('greece')) return 'https://images.unsplash.com/photo-1570077188670-e3a8d69ac5ff?q=80&w=400';
          if (c.contains('rome') || c.contains('italy')) return 'https://images.unsplash.com/photo-1552832230-c0197dd311b5?q=80&w=400';
          if (c.contains('barcelona') || c.contains('spain')) return 'https://images.unsplash.com/photo-1539037116277-4db20889f2d4?q=80&w=400';
          if (c.contains('zermatt') || c.contains('swiss') || c.contains('alps')) return 'https://images.unsplash.com/photo-1531366936337-7c912a4589a7?q=80&w=400';
          if (c.contains('sydney') || c.contains('australia')) return 'https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?q=80&w=400';
          if (c.contains('singapore')) return 'https://images.unsplash.com/photo-1525625293386-3f8f99389edd?q=80&w=400';
          if (c.contains('bangkok') || c.contains('thailand')) return 'https://images.unsplash.com/photo-1508009603885-50cf7c579365?q=80&w=400';
          if (c.contains('new york') || c.contains('york')) return 'https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?q=80&w=400';
          // Generic beautiful destination fallback
