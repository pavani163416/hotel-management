...
export function buildCspDirectives({ nonce, includeSwagger = false } = {}) {
  ...
  const directives = [
    ...
    `connect-src 'self' ${RAZORPAY_ORIGINS.join(" ")} ${GOOGLE_ORIGINS.join(" ")} https://countriesnow.space https://ipapi.co https://freeipapi.com   ws://localhost:5000 http://localhost:5000`,
